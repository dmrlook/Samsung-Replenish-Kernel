...
unsigned int fg_read_soc(void)
{
	...
	else 
	{
...
#elif defined(CONFIG_MACH_GIO)
			adj_soc = (((data[0]*10 + (data[1]>>7)*5 ) - 4) * 100) / (937 - 4);
#elif defined(CONFIG_MACH_RANT3)
			adj_soc = (((data[0]*10 + (data[1]>>7)*5 ) - 4) * 100) / (948 - 4);
#else
		...
		#if defined(CONFIG_MACH_GIO) || defined(CONFIG_MACH_RANT3)
